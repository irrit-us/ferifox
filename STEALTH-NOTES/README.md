...
- **Remaining personal-information interfaces should be tracked explicitly** — MediaDevices, Permissions, storage persistence/quota behavior beyond `estimate()`, Speech voices, WebGPU capabilities, media codecs, WebRTC SDP/stats, peripheral APIs, and protocol/runtime state need persona-coherent future work tied to Cloudflare's documented JavaScript, browser-signal, request, heuristic, and TLS layers
